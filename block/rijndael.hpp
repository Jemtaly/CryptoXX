...
protected:
    ...
    static constexpr auto IDEKT = []() {
        std::array<uint8_t, 256> IDEKT = {};
        for (int i = 0; i < 256; i++) {
            IDEKT[i] = i;
        }
        return IDEKT;
    ...
    static constexpr auto MCT_D = generate_LUT({.b = {0x0e, 0x09, 0x0d, 0x0b}}, IDEKT); // LUT for InvMixColumns step only
