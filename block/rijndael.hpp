...
protected:
    ...
    static constexpr uint8_t O_BOX[256] = {
    ...
    static constexpr auto generate_LUT = [](RijndaelClmn X, uint8_t const *B) {
        std::array<std::array<RijndaelClmn, 256>, 4> LUT = {};
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 256; j++) {
                uint8_t p = coef_mult(X.b[i], B[j]);
                for (int k = 0; k < 4; k++) {
                    LUT[k][j].b[(i + k) % 4] = p;
                }
            }
    ...
    static constexpr auto LUT_X = generate_LUT({.b = {0x0e, 0x09, 0x0d, 0x0b}}, O_BOX); // LUT for InvMixColumns step only
};
...
public:
    ...
    RijndaelTmpl(const uint8_t *mk) {
        ...
        for (int i = 0; i < R + 1; ++i) {
            ik[i][0].w = LUT_X[0][rk[i][0].b[0]].w ^ LUT_X[1][rk[i][0].b[1]].w ^ LUT_X[2][rk[i][0].b[2]].w ^ LUT_X[3][rk[i][0].b[3]].w;
            ik[i][1].w = LUT_X[0][rk[i][1].b[0]].w ^ LUT_X[1][rk[i][1].b[1]].w ^ LUT_X[2][rk[i][1].b[2]].w ^ LUT_X[3][rk[i][1].b[3]].w;
            ik[i][2].w = LUT_X[0][rk[i][2].b[0]].w ^ LUT_X[1][rk[i][2].b[1]].w ^ LUT_X[2][rk[i][2].b[2]].w ^ LUT_X[3][rk[i][2].b[3]].w;
            ik[i][3].w = LUT_X[0][rk[i][3].b[0]].w ^ LUT_X[1][rk[i][3].b[1]].w ^ LUT_X[2][rk[i][3].b[2]].w ^ LUT_X[3][rk[i][3].b[3]].w;
        }
    }
...
using AES192 = RijndaelTmpl<6>;
using AES256 = RijndaelTmpl<8>;
