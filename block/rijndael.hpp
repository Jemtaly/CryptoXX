...
protected:
    ...
    static constexpr uint8_t IDBOX[256] = {
    ...
    static constexpr auto generate_LUT = [](RijndaelClmn poly, uint8_t const *S_BOX) {
        // LUT[k][j] = poly * S_BOX[j] <<< k * 8
        std::array<std::array<RijndaelClmn, 256>, 4> LUT = {};
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 256; j++) {
                uint8_t p = RGF_multiply(poly.b[i], S_BOX[j]);
                for (int k = 0; k < 4; k++) {
                    LUT[k][j].b[(i + k) % 4] = p;
                }
            }
    ...
    static constexpr auto MCT_D = generate_LUT({.b = {0x0e, 0x09, 0x0d, 0x0b}}, IDBOX); // LUT for InvMixColumns step only
