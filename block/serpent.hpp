...
#define XXROTL(a, b, c, d) {                  \
    x[a] = ROTL(x[a]                   , 13); \
    x[c] = ROTL(x[c]                   ,  3); \
    x[d] = ROTL(x[d] ^ x[c] ^ x[a] << 3,  7); \
    x[b] = ROTL(x[b] ^ x[a] ^ x[c]     ,  1); \
    x[a] = ROTL(x[a] ^ x[b] ^ x[d]     ,  5); \
    x[c] = ROTL(x[c] ^ x[d] ^ x[b] << 7, 22); \
}
#define XXROTR(a, b, c, d) {                  \
    x[c] = ROTR(x[c], 22) ^ x[d] ^ x[b] << 7; \
    x[a] = ROTR(x[a],  5) ^ x[b] ^ x[d]     ; \
    x[b] = ROTR(x[b],  1) ^ x[a] ^ x[c]     ; \
    x[d] = ROTR(x[d],  7) ^ x[c] ^ x[a] << 3; \
    x[c] = ROTR(x[c],  3)                   ; \
    x[a] = ROTR(x[a], 13)                   ; \
}
